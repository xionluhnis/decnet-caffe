...
template<typename Dtype>
void DataTransformer<Dtype>::TransformImgImgAndSeg(const std::vector<cv::Mat>& cv_img_seg,
  Blob<Dtype>* transformed_data_blob, Blob<Dtype>* transformed_data2_blob, Blob<Dtype>* transformed_label_blob, const int ignore_label) {
  CHECK(cv_img_seg.size() == 3) << "Input must contain 2 images and 1 seg.";
  ...
  const int seg_channels = cv_img_seg[2].channels();
  int seg_height   = cv_img_seg[2].rows;
  int seg_width    = cv_img_seg[2].cols;
  ...
  int h_off = 0;
  int w_off = 0;
  cv::Mat cv_cropped_img1 = cv_img_seg[0];
  cv::Mat cv_cropped_img2 = cv_img_seg[1];
  cv::Mat cv_cropped_seg = cv_img_seg[2];

  // transform to double, since we will pad mean pixel values
  cv_cropped_img1.convertTo(cv_cropped_img1, CV_64F);
  cv_cropped_img2.convertTo(cv_cropped_img2, CV_64F);
  ...
  int pad_width  = std::max(crop_size - img_width, 0);
  if (pad_height > 0 || pad_width > 0) {
    cv::copyMakeBorder(cv_cropped_img1, cv_cropped_img1, 0, pad_height,
          0, pad_width, cv::BORDER_CONSTANT,
          cv::Scalar(mean_values_[0], mean_values_[1], mean_values_[2]));
    cv::copyMakeBorder(cv_cropped_img2, cv_cropped_img2, 0, pad_height,
          0, pad_width, cv::BORDER_CONSTANT,
          cv::Scalar(mean_values_[0], mean_values_[1], mean_values_[2]));
    cv::copyMakeBorder(cv_cropped_seg, cv_cropped_seg, 0, pad_height,
          0, pad_width, cv::BORDER_CONSTANT,
                       cv::Scalar(ignore_label));
    // update height/width
    img_height   = cv_cropped_img1.rows;
    img_width    = cv_cropped_img1.cols;
  ...
  if (crop_size) {
    ...
    cv::Rect roi(w_off, h_off, crop_size, crop_size);
    cv_cropped_img1 = cv_cropped_img1(roi);
    cv_cropped_img2 = cv_cropped_img2(roi);
    cv_cropped_seg = cv_cropped_seg(roi);
  }

  CHECK(cv_cropped_img1.data);
  CHECK(cv_cropped_img2.data);
  CHECK(cv_cropped_seg.data);

  Dtype* transformed_data1  = transformed_data_blob->mutable_cpu_data();
  Dtype* transformed_data2  = transformed_data2_blob->mutable_cpu_data();
  Dtype* transformed_label = transformed_label_blob->mutable_cpu_data();

  int top_index;
  const double* data1_ptr, *data2_ptr;
  const uchar* label_ptr;

  for (int h = 0; h < data_height; ++h) {
    data1_ptr = cv_cropped_img1.ptr<double>(h);
    data2_ptr = cv_cropped_img2.ptr<double>(h);
    label_ptr = cv_cropped_seg.ptr<uchar>(h);

    int data1_index = 0, data2_index = 0;
    int label_index = 0;
    ...
    for (int w = 0; w < data_width; ++w) {
      ...
      for (int c = 0; c < img_channels; ++c) {
        ...
        Dtype pixel1 = static_cast<Dtype>(data1_ptr[data1_index++]);
        Dtype pixel2 = static_cast<Dtype>(data2_ptr[data2_index++]);
        if (has_mean_file) {
          int mean_index = (c * img_height + h_off + h) * img_width + w_off + w;
          transformed_data1[top_index] =
            (pixel1 - mean[mean_index]) * scale;
          transformed_data2[top_index] =
            (pixel2 - mean[mean_index]) * scale;
        } else {
          if (has_mean_values) {
            transformed_data1[top_index] =
              (pixel1 - mean_values_[c]) * scale;
            transformed_data2[top_index] =
              (pixel2 - mean_values_[c]) * scale;
          } else {
            transformed_data1[top_index] = pixel1 * scale;
